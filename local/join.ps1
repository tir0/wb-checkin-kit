...
if ($Notify) {
    # 措辞要与实现一致：多账号步骤没有开 NOTIFY_ON_SUCCESS，朋友的账号
    # **只在需要他本人处理时**才推（凭据失效/签到失败），签上不打扰。
    Say '  3. 通知：已配置你自己的机器人 —— 只在需要你处理时才推给你'
    Say '     （凭据过期、签到失败）；每天签上不会来打扰。'
}
else {
    Say '  3. 通知：未配置。出问题（凭据过期/签到失败）时会推给仓库主的机器人。'
    Say '     想自己也收到，重跑本脚本并加 -Notify ''你的机器人地址''。'
}
Say ''
Say '什么时候要重跑'
